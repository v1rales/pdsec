...
Ответ: `{"result": "Клиент И. И. И., паспорт 45** ****56, тел +7 *** ***-**-**"}`.
...
curl -X POST http://127.0.0.1:8000/process \
  -H "Content-Type: application/json" \
  -d '{"payload":"Клиент И. И. И., паспорт 45** ****56, тел +7 *** ***-**-**","payload_id":"demo-1"}'
```
...
## Настройка систем (X-System-Id)

Заголовок `X-System-Id` выбирает политику системы (опционален, при отсутствии — `default_system`):

```bash
curl -X POST http://127.0.0.1:8000/process \
  -H "Content-Type: application/json" \
  -H "X-System-Id: system-a" \
  -d '{"payload":"Клиент Иванов Иван Иванович, паспорт 4509 123456, ИНН 500100732259","payload_id":"demo-4"}'
```

- `system-a` маскирует только fio/passport/phone/email/card (ИНН не маскирует).
- `system-b` (enabled:false) маскирует, но **не демаскирует** (получает только маску).
- Система вне allowlist → payload без маски (200).
...
## Водительское удостоверение (в/у)

```bash
curl -X POST http://127.0.0.1:8000/process \
  -H "Content-Type: application/json" \
  -d '{"payload":"водительское удостоверение серия 7712 номер 345678","payload_id":"demo-5"}'
```

Ответ: `{"result": "водительское удостоверение серия **** номер ******"}`.

## Где посмотреть логи и метрики

- **Логи**: выводятся в консоль uvicorn и в `logs/app.log` (ротация 10MB×5). Содержат сводку типов ПД на запрос (INFO) и поспановые детали (DEBUG), без значений ПД.
- **Метрики**: `GET /metrics` → `{requests_total, rps, avg_latency_seconds, latency_p50/p95/p99, tokens_total, masked_types_total}`.
